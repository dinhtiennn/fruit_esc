...
class EImage extends BasicESCArg<EImage> {
  ...
  @override
  Uint8List header() {
    return Uint8List.fromList(_compress ? [0x1F, 0x00] : [0x1D, 0x76, 0x30]);
  }
  ...
  CommandClause clause() {
    ...
    // Some render pipelines (especially after resize/anti-alias) can make QR/text too "light",
    // producing almost-all-zero raster data (blank print). If that happens, retry with a higher
    // threshold to capture more gray pixels as black.
    ...
    if (_compress) {
      return BinaryCommand(header())
          ...
          .appendNumber(height % 256)
          .appendNumber((fimage.data.length - 2) >> 24 & 0xff)
          .appendNumber((fimage.data.length - 2) >> 16 & 0xff)
          .appendNumber((fimage.data.length - 2) >> 8 & 0xff)
          .appendNumber((fimage.data.length - 2) & 0xff)
          .appendListInt(fimage.data.sublist(2))
          .clause();
    }
